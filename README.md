...
`render_template.py` script before being injected into the relevant template. These `DM_` environment variables are put
there by the PaaS, according to the manifests that control each service running there - see the [DM AWS
repo](https://github.com/alphagov/digitalmarketplace-aws/tree/master/paas).
